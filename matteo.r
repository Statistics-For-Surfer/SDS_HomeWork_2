...
gif_generator <- function(grid_t, cor_type, bonf, display=TRUE, save=TRUE){
  ...
  gif_path <- paste0("images/gifs/gif_", cor_type, "_cor_", bonf,"/")
  dir.create(file.path(gif_path), showWarnings = FALSE)
  unlink(paste0(gif_path, '*'))
  
  for(t in grid_t){
    path <- paste0("images/gifs/gif_", cor_type, "_cor_", bonf, "/t=0_",t*100,".jpg")
    if(t == .05){path <-  paste0("images/gifs/gif_", cor_type, "_cor_", bonf, "/t=0_0",t*100,".jpg")}
...
grid_t <- seq(0, 0.6, by=.05)

gif_generator(grid_t, cor_type = 'normal', bonf = T, save = T, display=T)
